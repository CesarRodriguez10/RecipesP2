...
class AddRecipeViewModel : ViewModel() {
    ...
    fun saveRecipe(name: String, description: String, category: String,
                   prepTime: Int, ingredients: List<String>, steps: List<String>, image_url: String? = null) {
        if (name.isBlank()) {
            _error.value = "El nombre no puede estar vacío"
            return
        }
        if (ingredients.isEmpty()) {
            _error.value = "Agrega al menos un ingrediente"
            return
        }
        if (steps.isEmpty()) {
            _error.value = "Agrega al menos un paso"
            return
        }
        val recipe = Recipe(
            name = name,
            ...
            steps = steps,
            image_url = image_url  // ← esto faltaba
        )
        viewModelScope.launch {
            _loading.value = true  // ← esto faltaba
